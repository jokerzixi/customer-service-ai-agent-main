...
def create_langgraph_thread() -> Optional[str]:
    """创建新的 LangGraph 线程，成功返回 thread_id。"""
...
def _looks_like_langgraph_thread_id(sid: Optional[str]) -> bool:
    if not sid or sid in ("default",):
        return False
    # 前端临时占位 id（web_时间戳）不是 LangGraph thread
    if str(sid).startswith("web_"):
        return False
    return True


def resolve_or_create_thread(client_session_id: Optional[str] = None) -> Optional[str]:
    """
    解析可用 thread_id：合法则复用，否则新建。
    返回 thread_id；失败返回 None。
    """
    global _current_thread_id

    sid = client_session_id if _looks_like_langgraph_thread_id(client_session_id) else None
    if sid:
        try:
    ...
    return create_langgraph_thread()
...
def ensure_thread_exists(client_session_id: Optional[str] = None) -> bool:
    ...
    """
    return resolve_or_create_thread(client_session_id) is not None
...
def run_chat_sync(
    ...
    thread_id = resolve_or_create_thread(client_session_id)
    if not thread_id:
        return None, '无法创建线程', 500, None

    assert _assistant_id
...
def fetch_thread_conversation_history(thread_id: str) -> List[Dict[str, Any]]:
    """从 LangGraph thread state 拉取对话历史。"""
    try:
        state_response = requests.get(
            f"{LANGGRAPH_API_URL}/threads/{thread_id}/state",
            timeout=10
        ...
        if state_response.status_code != 200:
            return []
        return conversation_history_from_state_data(state_response.json())
    except Exception as e:
        print(f"⚠️ 拉取线程对话失败: {e}")
        return []
...
def stream_chat_events(user_message: str, client_session_id: Optional[str] = None) -> Iterable[str]:
    ...
    assert _assistant_id is not None
    _current_thread_id = thread_id
