...
当前查询：{customer_query}"""
    ...
    def _format_category(self, category: str, policies: Dict[str, str], limit: int | None = None) -> str:
        lines = [f"【{category}】"]
        items = list(policies.items())
        if limit is not None:
            items = items[:limit]
        for policy, description in items:
    ...
    def _match_billing_info(self, query: str) -> str:
        ...
        for category, policies in self.billing_database.items():
            if any(keyword in query_lower for keyword in category.lower().split()):
                ...
                matched_info.append(self._format_category(category, policies))

        # 关键词 → 类目（避免误加全部类目，也避免解析【】失败）
        keyword_to_category = {
            "退款": "退款政策",
            "到账": "退款政策",
            "发票": "发票服务",
        ...
        if not matched_info:
            for keyword, category in keyword_to_category.items():
                ...
                if category in matched_categories:
                    continue
                policies = self.billing_database.get(category)
                if not policies:
                    continue
                matched_categories.append(category)
                matched_info.append(self._format_category(category, policies, limit=2))
